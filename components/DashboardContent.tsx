    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              <div className="divide-y divide-slate-50 min-h-[180px]">{operacionesEnRiesgoList.map((op) => (<div key={op.id} className="p-3 hover:bg-slate-50 cursor-pointer transition-colors" onClick={() => onNavigate('operacion_detalle', op.id)}><div className="flex items-center justify-between mb-1"><span className="text-sm font-medium text-slate-800">{op.id}</span><span className={`text-[10px] font-semibold px-2 py-0.5 rounded ${op.riesgo === 'critico' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{op.riesgo.toUpperCase()}</span></div><p className="text-xs text-slate-500 mb-1">{op.nombre}</p>{op.alertas[0] && <p className="text-xs text-red-600 leading-snug">{op.alertas[0]}</p>}</div>))}{operacionesEnRiesgoList.length === 0 && <div className="p-6 text-center text-slate-400 text-sm">✓ Sin operaciones en riesgo</div>}</div>
            </div>
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="p-4 border-b border-slate-50 flex items-center justify-between"><div className="flex items-center gap-2"><FileWarning className="text-slate-400" size={18} /><h2 className="font-semibold text-slate-800 text-sm">Docs Faltantes</h2></div></div>
              <div className="p-3 space-y-2 min-h-[180px]">{docsFaltantesAgrupados.map(([opId, docs], idx) => (<div key={idx} className="flex items-center justify-between text-sm hover:bg-slate-50 p-2 rounded cursor-pointer" onClick={() => onNavigate('operacion_detalle', opId)}><span className="text-slate-700 font-medium">{opId}</span><div className="flex items-center gap-1 flex-wrap justify-end">{docs.map((doc, i) => (<span key={i} className="text-[10px] bg-red-50 text-red-600 px-1.5 py-0.5 rounded">{doc}</span>))}</div></div>))}{docsFaltantesAgrupados.length === 0 && <p className="text-sm text-slate-400 text-center py-4">✓ Todos los docs al día</p>}</div>
            </div>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-50 flex items-center justify-between bg-amber-50/60"><div className="flex items-center gap-2"><AlertTriangle className="text-amber-600" size={20} /><h2 className="font-semibold text-amber-800">Stock Crítico</h2></div><button onClick={() => onNavigate('inventario')} className="text-sm text-amber-700 hover:text-amber-800 font-medium">Ver →</button></div>
            <div className="divide-y divide-slate-50">{stockCritico.map((item, idx) => (<div key={idx} className="p-3 flex items-center justify-between"><span className="text-sm text-slate-700">{item.nombre}</span><span className={`text-sm font-semibold ${item.stock === 0 ? 'text-red-600' : 'text-amber-600'}`}>{item.stock} uds</span></div>))}{stockCritico.length === 0 && <div className="p-4 text-center text-slate-400 text-sm">✓ Stock OK</div>}</div>
          </div>
        </div>
      </div>
    </div>
